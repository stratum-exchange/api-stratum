...
const model = {
  ...
  async _getAssetPrices(tokenList, pairs) {
    try {
      ...
      const url = `https://api.coingecko.com/api/v3/simple/price?ids=meter-stable,meter,usd-coin,tether&vs_currencies=USD`
      ...
      const usdtPrice = priceList.filter((asset) => {
        return asset.symbol === 'tether'
      }).reduce((asset) => {
        return asset.quote_rate
      })

      const mtrPrice = priceList.filter((asset) => {
        return asset.symbol === 'meter-stable'
      }).reduce((asset) => {
        return asset.quote_rate
      })

      const mtrgPrice = priceList.filter((asset) => {
      ...
      const tokenListWithPrices = tokenList.map((token) => {
        ...
        if(token.address.toLowerCase() === config.usdt.address.toLowerCase()) {
          token.priceUSD = usdtPrice.quote_rate
          return token
        }
        ...
        if(maxLiquidityPair.token0.address.toLowerCase() === token.address.toLowerCase()) {
          if(maxLiquidityPair.token1.address.toLowerCase() === config.wmtr.address.toLowerCase()) {
          ...
          if(maxLiquidityPair.token0.address.toLowerCase() === config.wmtr.address.toLowerCase()) {
            pairedAssetPrice = mtrPrice.quote_rate
          }
    ...
    try {
      const relevantPairs = pairs.filter((pair) => {
        return (
            ...
            (pair.token0.address.toLowerCase() == config.wmtr.address.toLowerCase() || pair.token1.address.toLowerCase() == config.wmtr.address.toLowerCase() ||
